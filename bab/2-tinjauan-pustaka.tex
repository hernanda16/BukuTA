...
Penelitian \textcite{Dharma2025PerformanceEvaluation} menjelaskan bahwa performa \emph{DeepLabV3 Plus} dengan \emph{backbone} ResNet-50 yang dilatih dengan dataset lokal ITS pada malam hari mendapatkan performa paling baik mencapai \emph{mean Intersection over Union} (mIoU) 76,45\% dan \emph{Pixel Accuracy} 99,64\% pada kelas jalan.
Sementara itu, penelitian \textcite{wang2023internimage} mengusulkan \emph{InternImage} sebagai \emph{backbone} segmentasi berbasis \emph{Deformable Convolution v3} (DCNv3) yang dipralatih dengan dataset global Mapillary Vistas dan dilakukan proses \emph{fine-tuning} pada dataset global Cityscapes, mendapatkan mIoU yang lebih tinggi yaitu 87,0\% pada dataset validasi Cityscapes.
...
Hasil penelitian \textcite{Zazuli2024SensorFusion} mendapatkan rata-rata eror 0.43 m pada laju kontrol 50 Hz dengan menggabungkan data sensor GNSS, odometri roda, dan IMU dengan metode \emph{Extended Kalman Filter} (EKF).
...
Perbedaan ini ditunjukkan pada Gambar \ref{fig:segmentasi_visual} dimana semua piksel dilabeli berdasarkan kelasnya.
...
Model ini menempati peringkat pertama di antara model \emph{open-source} segmentasi pada \emph{benchmark} Cityscapes pada kategori mIoU \emph{class} sehingga digunakan pada penelitian ini.
...
Berbeda dengan kuantisasi INT8 yang memerlukan kalibrasi, FP16 tidak memerlukan dataset kalibrasi dan mempertahankan akurasi yang sangat dekat dengan presisi penuh \parencite{zhou2022exploring}.
...
Pada persamaan \eqref{eq:kf_predict_x}, $A$ merupakan matriks transisi sistem yang memproyeksikan \emph{state} dari langkah waktu sebelumnya ke langkah waktu sekarang, $B$ merupakan matriks masukan yang memetakan pengaruh vektor masukan terhadap perubahan \emph{state}, $\hat{x}_k^-$ adalah \emph{state} hasil prediksi (\emph{priori}), $\hat{x}_{k-1}$ adalah \emph{state} koreksi sebelumnya (\emph{posteriori}), dan $u_{k-1}$ adalah vektor masukan dari sensor inersia.
Persamaan \eqref{eq:kf_predict_p} disebut \emph{Covariance Extrapolation Equation}, di mana $P_k^-$ merupakan kovarians estimasi \emph{priori}, $P_{k-1}$ merupakan kovarians estimasi sebelumnya, dan $Q$ merupakan kovarians \emph{process noise} yang mewakili ketidakpastian model sistem. Kovarians selalu ikut dipropagasi karena Kalman Filter memperlakukan \emph{state} sebagai variabel acak berdistribusi normal \parencite{Becker2024KF}.
...
Pada persamaan tersebut, $z_k$ merupakan vektor pengukuran dari sensor, $C$ merupakan matriks observasi yang memetakan \emph{state} ke ruang pengukuran, $R$ merupakan kovarians \emph{measurement noise}, dan $K_k$ merupakan \emph{Kalman Gain}.

\emph{Kalman Gain} ($K$) merupakan inti dari Kalman Filter.
Nilainya dihitung ulang pada setiap iterasi berdasarkan $P_k^-$ dan $R$ \parencite{Becker2024KF}.
Jika $R$ kecil (pengukuran akurat), $K$ membesar sehingga estimasi lebih bergantung pada pengukuran.
Sebaliknya, jika $P^-$ kecil (prediksi akurat), $K$ mengecil sehingga estimasi lebih bergantung pada prediksi.
...
\begin{equation}
  P_k = (I - K_k C) P_k^- (I - K_k C)^\top + K_k R K_k^\top,
  \label{eq:kf_cov_update}
\end{equation}

Pada persamaan tersebut, $I$ merupakan matriks identitas. Bentuk Joseph menjamin $P_k$ tetap simetris dan \emph{positive semi-definite} selama $R$ \emph{positive semi-definite}, sehingga mencegah divergensi numerik pada implementasi \parencite{Barfoot2017}.
