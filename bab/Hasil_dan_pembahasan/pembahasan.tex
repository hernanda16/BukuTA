...
Pengujian terhadap data aktual posisi menunjukkan bahwa DLIO memiliki RMSE terendah (0,134 m) di antara keempat metode yang diuji, menjelaskan pemilihan DLIO sebagai \emph{baseline} untuk evaluasi yang lebih kompleks.
...
Pada estimasi kecepatan, terjadi perbedaan karakteristik yaitu data DLIO justru memiliki RMSE tertinggi (0,166 m/s) sementara Kalman Filter, GPS, dan odometri menunjukkan performa setara (0,133--0,138 m/s).
...
Pengujian pada 6 rute dengan panjang berbeda menunjukkan bahwa Kalman Filter secara konsisten memiliki RMSE lebih rendah daripada GPS pada seluruh rute.
Pada rute terpanjang robotika-k1mart (3431,6 m), RMSE Kalman Filter 0,88 m berhasil menekan akumulasi galat yang mencapai 8,59 m pada odometri murni.
Hasil ini membuktikan efektivitas fusi sensor dalam membatasi pertumbuhan galat seiring bertambahnya jarak tempuh dan slip yang disebabkan oleh kondisi jalan yang tidak ideal.

Analisis pada area Rektorat juga memberikan bukti ketahanan Kalman Filter terhadap degradasi GPS.
Pada 9 iterasi dengan variasi waktu henti, GPS mengalami \emph{drift} signifikan dengan RMSE mencapai 9,43 m dengan waktu henti 20,8 detik, sementara Kalman Filter mempertahankan RMSE dalam rentang 1,07--1,56 m.
Pola ini mengonfirmasi bahwa mekanisme fusi sensor pada Kalman Filter mampu meredam dampak \emph{multipath} dan \emph{drift} GPS yang umum terjadi di area dengan gedung tinggi, sebagaimana dilaporkan oleh \textcite{Lee2022ResilientNavigation}.
